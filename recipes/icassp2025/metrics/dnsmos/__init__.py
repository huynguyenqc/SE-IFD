...
class ComputeScore:
    ...
    def __call__(self, fpath, sampling_rate, is_personalized_MOS):
        aud, input_fs = sf.read(fpath, always_2d=True)
        aud = aud[:, -1]
        fs = sampling_rate
        if input_fs != fs:
...
def main(
        clips: List[str],
        p808_model_path: str = str(Path(__file__).parent /  'model_v8.onnx'),
        primary_model_path: str = str(Path(__file__).parent / 'sig_bak_ovr.onnx'),
